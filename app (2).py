...
Supports both random allocation and TF-IDF semantic similarity matching.
"""
...
def run_allocation(master: pd.DataFrame, reviews_per_person: int, seed: int,
    ...
    if mode == "semantic":
        if "ApplicationText" not in master.columns or master["ApplicationText"].fillna("").str.strip().eq("").all():
            raise ValueError("Semantic mode requires a non-empty 'ApplicationText' column.")
    ...
    quality = {}
    if mode == "semantic" and sim_matrix is not None:
...
with st.sidebar:
    ...
    mode_label = st.radio(
        "Allocation mode",
        ["🎲 Random", "🔍 Semantic similarity"],
    )
    mode = "semantic" if "Semantic" in mode_label else "random"
...
The default `122` matches the original R script.
    ...
    if mode == "semantic":
        st.markdown("**ApplicationText** — Full application text *(required for semantic mode)*")
...
with st.expander("ℹ️  How does this work?", expanded=False):
    ...
    if mode == "semantic":
        st.info("""
**About TF-IDF semantic matching**

TF-IDF (Term Frequency–Inverse Document Frequency) represents each application as a weighted word-count vector,
down-weighting words common across all applications and up-weighting rare, distinctive terms.
Cosine similarity between two vectors measures topical alignment regardless of document length.
The algorithm greedily assigns each reviewer their highest-similarity available reviewees,
processing the most-constrained reviewers first to minimise incomplete assignments.
A similarity score of 1.0 means identical vocabulary; 0.0 means no shared distinctive terms.
        """)

st.markdown("---")
st.markdown(f"**Mode:** {'🔍 Semantic similarity matching (TF-IDF)' if mode == 'semantic' else '🎲 Random allocation'}")
st.markdown("---")
